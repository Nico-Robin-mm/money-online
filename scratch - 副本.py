...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def checkAvailPlayer(self):
        wsClient.sendMsg(self.path)

    def selectOppo(self):
        httpd.oppo = self.path[len(SELECT_OPPO) + 1:]
        wsClient.sendMsg(SELECT_OPPO + '/' + httpd.myName + '/' + httpd.oppo)

    def respOppo(self):
        res = self.path[len(RESP_OPPO) + 1:]

        wsClient.sendMsg(RESP_OPPO+"/"+httpd.requestFrom+'/'+httpd.myName+"/"+res) # resp the requestor name, oppo name, and response

    def setName(self):
        httpd.myName = self.path[len(SET_NAME) + 1:]
        wsClient.sendMsg(self.path)
    ...
    def do_GET(self):
        ...
        if self.path == '/poll':
            output = 'isNameOk %s\nOrequestFrom %s\nAvailPlayer %s\nDice %s\ncharacter %s\nAllConnected %s\nAvailChar %s\n' % (
            httpd.isNameOk, httpd.requestFrom, httpd.availPlayer, httpd.dice, httpd.movingChar, httpd.allConnected, httpd.availChar)
            #   self.abcd()
            self.reply(output)
            # print (output)
            ...
            self.partnerMoved()
            self.reply('ok')
        elif SELECT_OPPO in self.path:  # select oppo
            self.selectOppo()
            self.reply('ok')
        elif RESP_OPPO in self.path:  # response to an oppo
            self.respOppo()
            self.reply('ok')

        elif SET_NAME in self.path:
            self.setName()
            self.reply('ok')


        elif self.path == CHECK_AVAIL_PLAYER:  # this is to reset
            self.checkAvailPlayer()
            self.reply('ok')
            ...
            self.end_headers()
            self.wfile.write(
                b'<cross-domain-policy><allow-access-from domain=\"*\" to-ports=\"12345\"/></cross-domain-policy>')
...
class WSClient:
    ...
    async def consumer(self, message):
        ...
        if message.startswith(CHECK_CHAR):
            ...
            handle_updateSelectedChar(message[len(SET_CHAR) + 1:])  # update the selected char param
        elif message.startswith(CHECK_AVAIL_PLAYER):
            handle_checkAvailPlayer(message[len(CHECK_AVAIL_PLAYER) + 1:])  # update the available player param
        elif message.startswith(SET_NAME):
            handle_setName(message[len(SET_NAME) + 1:])
        elif message.startswith(CHECK_OPPO_RES):
            handle_checkOppoRes(message[len(CHECK_OPPO_RES) + 1:])
        elif message.startswith(SELECT_OPPO):
            handle_selectOppo(message[len(SELECT_OPPO) + 1:])

def handle_selectOppo(res):
    httpd.availChar = res.split('>')[1]


def handle_checkOppoRes(res):
    httpd.requestFrom = res


def handle_setName(res):
    httpd.isNameOk = res


def handle_checkAvailPlayer(res):
    httpd.availPlayer = res
